...
def load_pi05_policy(pretrained_path="lerobot/pi05_base"):
    """Load PI0.5 and apply the attention mask patch.

    NOTE: We do NOT freeze params here. Freezing happens in vla.py AFTER
    DDP wrapping via prepare_model(), so both ranks see the same full model
    during DDP initialization. Freezing before DDP causes a param count
    mismatch across ranks on single-machine multi-GPU setups.
    """
    from lerobot.policies.pi05 import PI05Policy
